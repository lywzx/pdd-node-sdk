...
/**
 * Throttle test adapter
 * @param adapter
 * @param groupName
 */
export function testThrottleUtil(adapter: PddApiThrottleAdapter, groupName: string) {
  describe(`pdd-api-throttle test ${groupName}`, function () {
    ...
    beforeEach(function () {
      pddApiThrottleInstance = new PddApiThrottle(adapter, {
        timeout: 1000,
      });
